...
//UTF_8 转gb2312
void strCoding::UTF_8ToGB2312(string &pOut, const char *pText, int pLen)
{
     char buf[4];
	 int cLEN = pLen + (pLen >> 2) + 2;
	 cLEN = cLEN * 2;	//加大点，避免可能的堆栈损坏
     char* rst = new char[cLEN];
    memset(buf,0,4);
    memset(rst,0,cLEN);
...
//GB2312 转为 UTF-8
string strCoding::GB2312ToUTF_8(char *pText, int pLen){
	if (pLen<=0)
	{
		pLen = strlen((pText));
	}
	string strOut;
	GB2312ToUTF_8(strOut,pText,pLen);
	return strOut;
}
