...
//////////////////////////////////////////////////////////////////////////
//tianzuo,2008-6-6.解决在vc2003下编译通不过的问题
...
public:
	...
	INT_PTR DoModal(HWND hWndParent = ::GetActiveWindow(), LPARAM dwInitParam = NULL)
	{
		...
		m_bModal = true;
#endif //_DEBUG
		return ::DialogBoxIndirectParam(_AtlBaseModule.GetResourceInstance(), 
	...
	HWND Create(HWND hWndParent, LPARAM dwInitParam = NULL)
	{
		...
		m_bModal = false;
#endif //_DEBUG
		HWND hWnd = ::CreateDialogIndirectParam(_AtlBaseModule.GetResourceInstance(), ATL::_DialogSplitHelper::SplitDialogTemplate((LPDLGTEMPLATE)T::GetDialogTemplate(static_cast<T*>(this)->GetDialogStyle()), NULL),
			hWndParent, T::StartDialogProc, dwInitParam);
	...
	// default DialogTemplate, overidable
	static LPCDLGTEMPLATE GetDialogTemplate(DWORD dwDlgStyle)
	{
		...
		// Define a dialog box.
		lpdt->style = (dwDlgStyle & ~DS_SETFONT);
		...
		*lpw++ = 0;   // predefined dialog box class (by default)
		lpwsz = (LPWSTR) lpw;
		nchar = /*1+*/ MultiByteToWideChar (CP_ACP, 0, "DHTML Dialog", -1, lpwsz, 2048);
		lpw   += nchar;
		lpw = lpwAlign (lpw); // align DLGITEMTEMPLATE on DWORD boundary

		////-----------------------
		//// Define an WebBrowser Control.
		////-----------------------
		//lpdit = (LPDLGITEMTEMPLATE) lpw;
		//lpdit->x  = 0; lpdit->y  = 0;
		//lpdit->cx = 300; lpdit->cy = 300;
		//lpdit->id = ID__WEBBROWSER;  // WebBrowser Control identifier
		//lpdit->style = WS_CHILD | WS_VISIBLE;

		//lpw = (LPWORD) (lpdit + 1);
		//lpwsz = (LPWSTR) lpw;
		//nchar = /*1+*/ MultiByteToWideChar (CP_ACP, 0, "{8856F961-340A-11D0-A96B-00C04FD705A2}", -1, lpwsz, 2048); // webbrowser ActiveX
		//lpw   += nchar;
...
public:
	...
	BOOL LoadFromResource(LPCTSTR lpszResource)
	{
		...
		else
		{
			...
			sprintf(strResourceURL, _T("res://%s/%s"), lpszModule, lpszResource);
	...
	BOOL LoadFromResource(UINT nRes)
	{
		...
		if (GetModuleFileName(hInstance, lpszModule, _MAX_PATH))
		{
			...
			sprintf(strResourceURL, _T("res://%s/%d"), lpszModule, nRes);
			Navigate(strResourceURL, 0, 0, 0);
		}
		else
			bRetVal = FALSE;
...
public:
	...
	void OnCenterWindow(VARIANT* para1=NULL, VARIANT* =NULL , VARIANT* =NULL) { CenterWindow(); }
	void OnResizeWindow(VARIANT* para1=NULL, VARIANT* =NULL , VARIANT* =NULL)
	{
		long width = 0;
		if (para1)
		{
	...
	void ResizeWindow(long width=0)
	{
		...
		if (spBody2)
		{
			...
			if (_cy<=0)
			{
				...
				RECT rcBrowser;
				m_wndBrowser.GetWindowRect(&rcBrowser);
